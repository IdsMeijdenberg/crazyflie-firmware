...
#define PID_ALTITUDE_OFFSET 44705.0
...
#define Z_REF (-0.25f) 				// Hard-coded altitude reference position
...
static void DynamicEquationAltitude(altitude_state_t *state, positionMeasurement_t *ext_pos, setpoint_t *setpoint, float dt)
{
	...
	state->input = (setpoint->thrust - PID_ALTITUDE_OFFSET)*MASS_CRAZYFLIE*GRAVITY/PID_ALTITUDE_OFFSET;

	float Y[2] = {0};
	float dY[2]= {0};
	float meas = ext_pos->z + Z_REF;
	...
	state->x_hat = Y[0];
	state->v_hat = Y[1];
	state->innovation_1 = -state->input/MASS_CRAZYFLIE;
	state->innovation_2 = dY[1];
	state->innovation_3 = meas - Y[0];
}
